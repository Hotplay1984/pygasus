...
def download_implied_rate(datadate):
	date_dt = dt.datetime.strptime(datadate, '%Y%m%d')
	date_str = date_dt.strftime('%Y-%m-%d')
	
	url_0 = 'http://www.chinamoney.com.cn/fe-c/dollarCurveHisortyAction.do?startDate=@@startDate&endDate=@@endDate&rmbRateSrc=&spotrateSrc=&bpSrc=&message=&RmbRateSrc=&SpotrateSrc=&BpSrc='
	url = url_0.replace('@@startDate', date_str)
	url = url.replace('@@endDate', date_str)
	
	bs = get_soup(url)
	table = bs.findAll('table')[5]
	...
	columns = ['rate_date', 'cny_rate_type', 'quote_type', 'pip_type', 'time_bucket', 
	  'usd_implied_rate', 'cny_rate', 'fx_rate','pips']
...
def send_data(df):
	...
	file_name = local_path + 'daily_update_cfets_usd_implied_rate.xls'
	df.to_excel(file_name)
	df.to_sql(
		'cfets_usd_implied_rate',
		engine,
		schema='cfets',
		if_exists = 'append',
		index = False
	...
	send_mail_via_com('hello Will', 
		...
		select_file = 'daily_update_cfets_usd_implied_rate.xls')
	
def main():
	print('Start downloading Cfets USD implied rates......')
	today_dt = dt.datetime.now()
	today_str = today_dt.strftime('%Y%m%d')
	# today_str = '20171030'
	try:
		df = download_implied_rate(today_str)
		send_data(df)
		print('finished!')
		os.system('pause')
	except:
